function [Q,T_pb, T_lbe, material] = energy(sealer_outer,flow_pb, L, tube_inner, tube_thick, n)
    ...
    Q = c_pb*flow_pb*(T_pb(1)-T_pb(end));               %Heat
    material = pi*(tube_outer^2-tube_inner^2)*L*N;      %Volume of material
    
    rho_hot = 10981.7-1.1369*(T_lbe(1)+273);
    rho_cold = 10981.7-1.1369*(T_lbe(end)+273);
    P = 9.82*50*(rho_cold-rho_hot);                     %Pressure drop
end
